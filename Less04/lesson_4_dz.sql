/* Задание.
Таблицы "shops" и "cats" с данными.
...
Таблицы "analysis", "groups", "orders" с данными.
...
-- Таблицы "shops" и "cats" с данными.
CREATE DATABASE IF NOT EXISTS lesson_4_dz;
USE lesson_4_dz; 
...
-- 1. Вывести всех котиков по магазинам по id 
-- (условие соединения shops.id = cats.shops_id)
SELECT s.id, s.shopname, c.`name` FROM shops AS s
JOIN cats AS c ON s.id = c.shops_id ORDER BY s.id;

-- 2. Вывести магазин, в котором продается кот “Мурзик” 
-- (попробуйте выполнить 2 способами)
-- 1 способ:
SELECT s.shopname, c.`name` FROM shops AS s
JOIN cats AS c ON s.id = c.shops_id
WHERE c.`name` = "Murzik";
-- 2 способ:
SELECT s.shopname FROM shops AS s
WHERE s.id IN (SELECT shops_id FROM cats WHERE `name` = "Murzik");

-- 3. Вывести магазины, в которых НЕ продаются коты “Мурзик” и “Zuza”
SELECT s.shopname, c.`name` FROM shops AS s
LEFT JOIN cats AS c ON s.id = c.shops_id
...
-- продавались 5 февраля 2020 и всю следующую неделю
SELECT 
	g.gr_name AS "Название группы", 
    a.an_name AS "Название анализа", 
    a.an_price AS "Розничная цена анализа"
FROM (analysis AS a 
JOIN orders AS o ON a.an_id = o.ord_an) 
JOIN groupsan AS g ON a.an_group = g.gr_id
WHERE o.ord_datetime >= "2020-02-05 07:05:14"
AND o.ord_datetime <= ('2020-02-05 07:05:14' + 
	INTERVAL 1 DAY + INTERVAL 1 WEEK);
